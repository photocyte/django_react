...
class QueryListCreate(generics.ListCreateAPIView):
    ...
    def execute_dna_to_protein_alignment(self,dna_seq):
        ...
        for k,v in preloaded_fastas.items():
            diamond_cmd='apptainer exec ./diamond_2.0.15--hb97b32f_1.sif diamond blastx --evalue 9999999 --query-gencode {gc} --db {db}'.format(gc=k,db=v) ##diamond will read the query in FASTA format from stdin.
